...
public class CellularFunction2D implements Function2D {
    ...
    for (int i = 0; i < numPoints; i++) {
      ...
      if (distancePower == 1.0f) d = dx + dy;
        ...
        d =
            (float)
                Math.pow(
                    Math.pow(dx, distancePower) + Math.pow(dy, distancePower), 1 / distancePower);
  ...
  public float evaluate(float x, float y) {
    ...
    float t = 0;
    for (int i = 0; i < 2; i++) t += coefficients[i] * results[i].distance;
    if (angular) t += Math.atan2(fy - results[0].y, fx - results[0].x) / (2 * Math.PI) + 0.5;
    return t;
  }
}
