...
/**
 * An experimental filter which can be used for keying against a clean shot. Given a source image, a
 * clean image and a destination image, the filter replaces all pixels in the source which nearly
 * equal the equivalent clean pixel by destination pixels.
 */
      ...
      for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
          ...
          if (Math.abs(hsb1[0] - hsb2[0]) < hTolerance
              ...
              && Math.abs(hsb1[2] - hsb2[2]) < bTolerance) inPixels[x] = out;
          else inPixels[x] = rgb1;
        }
