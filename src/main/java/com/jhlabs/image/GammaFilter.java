...
public class GammaFilter extends TransferFilter {
  ...
  protected void initialize() {
    ...
    if (gGamma == rGamma) gTable = rTable;
    else gTable = makeTable(gGamma);

    if (bGamma == rGamma) bTable = rTable;
    else if (bGamma == gGamma) bTable = gTable;
    else bTable = makeTable(bGamma);
  }
