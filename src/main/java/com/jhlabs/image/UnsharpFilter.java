  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        if (Math.abs(r1 - r2) >= threshold) r1 = PixelUtils.clamp((int) ((a + 1) * (r1 - r2) + r2));
        if (Math.abs(g1 - g2) >= threshold) g1 = PixelUtils.clamp((int) ((a + 1) * (g1 - g2) + g2));
        if (Math.abs(b1 - b2) >= threshold) b1 = PixelUtils.clamp((int) ((a + 1) * (b1 - b2) + b2));
