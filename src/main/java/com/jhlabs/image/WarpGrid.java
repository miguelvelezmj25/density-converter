...
public class WarpGrid {
  ...
  /**
   * Add a new row to the grid. "before" must be in the range 1..rows-1. i.e. you can only add rows
   * inside the grid.
   */
  ...
  /**
   * Add a new column to the grid. "before" must be in the range 1..cols-1. i.e. you can only add
  ...
  public void lerp(float t, WarpGrid destination, WarpGrid intermediate) {
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < cols; col++) {
        intermediate.xGrid[index] =
            (float) ImageMath.lerp(t, xGrid[index], destination.xGrid[index]);
        intermediate.yGrid[index] =
            (float) ImageMath.lerp(t, yGrid[index], destination.yGrid[index]);
        index++;
      }
    }
  }
  ...
  public void warp(
    ...
    try {
      ...
      for (v = 0; v < gridRows; v++) {
        interpolateSpline(
            sourceGrid.xGrid, sourceGrid.yGrid, offset, gridCols, splines.xGrid, offset2, cols);
        offset += gridCols;
        offset2 += cols;
      }
      ...
      for (v = 0; v < gridRows; v++) {
        interpolateSpline(
            destGrid.xGrid, destGrid.yGrid, offset, gridCols, splines.yGrid, offset2, cols);
        offset += gridCols;
        offset2 += cols;
      }
  ...
  protected void interpolateSpline(
      float[] xKnots,
      float[] yKnots,
      int offset,
      int length,
    ...
    for (int i = 0; i < splineLength; i++) {
      if (index <= end && i > xKnots[index]) {
        ...
        x0 = xKnots[index];
        index++;
        if (index <= end) x1 = xKnots[index];
        if (index < end) k3 = yKnots[index + 1];
        else k3 = k2;
      }
      float t = (i - x0) / (x1 - x0);
  ...
  protected void interpolateSpline2(
      float[] xKnots,
      float[] yKnots,
      int offset,
      float[] splineY,
