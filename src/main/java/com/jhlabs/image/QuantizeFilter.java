...
/**
 * A filter which quantizes an image to a set number of colors - useful for producing images which
 * are to be encoded using an index color model. The filter can perform Floyd-Steinberg
 * error-diffusion dithering if required. At present, the quantization is done using an octtree
 * algorithm but I eventually hope to add more quantization methods such as median cut. Note: at
 * present, the filter produces an image which uses the RGB color model (because the application it
 * was written for required it). I hope to extend it to produce an IndexColorModel by request.
 */
public class QuantizeFilter extends WholeImageFilter {
  ...
  /**
   * Set whether to use a serpentine pattern for return or not. This can reduce 'avalanche'
