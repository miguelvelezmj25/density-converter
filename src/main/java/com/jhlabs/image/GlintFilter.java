...
public class GlintFilter extends AbstractBufferedImageOp {
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    if (blur != 0) mask = new GaussianFilter(blur).filter(mask, null);

    if (dst == null) dst = createCompatibleDestImage(src, null);
    int[] dstPixels;
    if (glintOnly) dstPixels = new int[width * height];
    else dstPixels = getRGB(src, 0, 0, width, height, null); // FIXME - only need 2*length
      ...
      for (int x = 0; x < width; x++) {
        if ((pixels[x] & 0xff) > threshold * 255) {
          ...
          for (int i = 0, k = 0; i <= xmax; i++, k++)
            dstPixels[index + i] =
                PixelUtils.combinePixels(dstPixels[index + i], colors[k], PixelUtils.ADD);
