...
/**
 * A Colormap implemented using Catmull-Rom colour splines. The map has a variable number of knots
 * with a minimum of four. The first and last knots give the tangent at the end of the spline, and
 * colours are interpolated from the second to the second-last knots. Each knot can be given a type
 * of interpolation. These are:
 *
 * <UL>
...
public class Gradient extends ArrayColormap implements Cloneable {
  ...
  public int knotAt(int x) {
    for (int i = 1; i < numKnots - 1; i++) if (xKnots[i + 1] > x) return i;
    return 1;
  }
