 ...
 * displacement image is provided and pixels in the source image are displaced according to the
 * gradient of the displacement map.
 */
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    for (int y = 0; y < dh; y++) {
      for (int x = 0; x < dw; x++) {
        ...
        int b = rgb & 0xff;
        mapPixels[i] =
            (r + g + b) / 8; // An arbitrary scaling factor which gives a good range for "amount"
        i++;
      }
    }
