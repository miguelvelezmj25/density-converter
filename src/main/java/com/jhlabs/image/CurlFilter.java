  ...
  */
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    for (int y = 0; y < outHeight; y++) {
      for (int x = 0; x < outWidth; x++) {
        ...
        if (out[3] != 0)
          outPixels[x] =
              PixelUtils.combinePixels(rgb, inPixels[srcWidth * y + x], PixelUtils.NORMAL);
        else outPixels[x] = rgb;
      }
  ...
  protected void transformInverse(int x, int y, float[] out) {
    ...
    */
    ...
    // Shade the curl
    float shade =
        !offpage && oncurl ? 1.9f * (1.0f - (float) Math.cos(Math.exp((qx - tx) / radius))) : 0;
    out[2] = 1 - shade;
