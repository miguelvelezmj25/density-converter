...
public class FlareFilter extends PointFilter {
  ...
  public int filterRGB(int x, int y, int rgb) {
    ...
    float a =
        (float) Math.exp(-distance * distance * gauss) * mix
    ...
    if (distance < radius - ringWidth || distance > radius + ringWidth) ring = 0;
    else {
    ...
    angle =
        (ImageMath.mod(angle / ImageMath.PI * 17 + 1.0f + Noise.noise1(angle * 10), 1.0f) - 0.5f)
            * 2;
