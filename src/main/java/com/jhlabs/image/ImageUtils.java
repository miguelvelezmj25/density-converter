...
public abstract class ImageUtils {
  ...
  public static BufferedImage createImage(ImageProducer producer) {
    ...
    if ((pg.status() & ImageObserver.ABORT) != 0) throw new RuntimeException("Image fetch aborted");
    if ((pg.status() & ImageObserver.ERROR) != 0) throw new RuntimeException("Image fetch error");
  ...
  /**
   * Convert an Image into a TYPE_INT_ARGB BufferedImage. If the image is already of this type, the
   ...
   */
  public static void paintCheckedBackground(
    ...
    if (backgroundImage != null) {
      ...
      if (r == null) r = new Rectangle(c.getSize());
      ...
      if (w != -1 && h != -1) {
        ...
        for (y = y1; y < y2; y += h)
          for (x = x1; x < x2; x += w) g.drawImage(backgroundImage, x, y, c);
      }
      g.setClip(saveClip);
    }
  }
  ...
  public static Rectangle getSelectedBounds(BufferedImage p) {
    ...
    if (anySelected) return new Rectangle(minX, minY, maxX - minX + 1, maxY - minY + 1);
    return null;
  }

  /**
   * Compose src onto dst using the alpha of sel to interpolate between the two. I can't think of a
   * way to do this using AlphaComposite.
   *
  ...
  public static int[] getRGB(
      BufferedImage image, int x, int y, int width, int height, int[] pixels) {
