...
/**
 * A filter which draws a coloured gradient. This is largely superceded by GradientPaint in Java1.2,
 * but does provide a few more gradient options.
 */
public class GradientFilter extends AbstractBufferedImageOp {
  ...
  private void linearGradient(int[] pixels, int y, int w, int h) {
    ...
    if (repeat) repeatGradient(pixels, w, h, rowrel, dx, dy);
    else singleGradient(pixels, w, h, rowrel, dx, dy);
  }
