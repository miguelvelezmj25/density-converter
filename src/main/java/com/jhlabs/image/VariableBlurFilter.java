...
/**
 * A filter which performs a box blur with a different blur radius at each pixel. The radius can
 * either be specified by providing a blur mask image or by overriding the blurRadiusAt method.
 */
public class VariableBlurFilter extends AbstractBufferedImageOp {
  ...
  public BufferedImage filter(BufferedImage src, BufferedImage dst) {
    ...
    if (dst == null) dst = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
    ...
    if (premultiplyAlpha) ImageMath.unpremultiply(inPixels, 0, inPixels.length);
  ...
  public void blur(int[] in, int[] out, int width, int height, int radius, int pass) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        int ra;
        if (blurMask != null) {
          if (pass == 1) ra = (int) ((mask[x] & 0xff) * hRadius / 255f);
          else ra = (int) ((mask[x] & 0xff) * vRadius / 255f);
        } else {
          if (pass == 1) ra = (int) (blurRadiusAt(x, y, width, height) * hRadius);
          else ra = (int) (blurRadiusAt(y, x, height, width) * vRadius);
        }
