...
public class ArrayColormap implements Colormap, Cloneable {
  ...
  /**
   * Set the color at "index" to "color". Entries are interpolated linearly from the existing
   * entries at "firstIndex" and "lastIndex" to the new entry. firstIndex < index < lastIndex must
   * hold.
   *
  ...
  public void setColorInterpolated(int index, int firstIndex, int lastIndex, int color) {
    ...
    for (int i = firstIndex; i <= index; i++)
      map[i] =
          ImageMath.mixColors((float) (i - firstIndex) / (index - firstIndex), firstColor, color);
  ...
  public void setColorRange(int firstIndex, int lastIndex, int color1, int color2) {
    for (int i = firstIndex; i <= lastIndex; i++)
      map[i] =
          ImageMath.mixColors((float) (i - firstIndex) / (lastIndex - firstIndex), color1, color2);
  }
