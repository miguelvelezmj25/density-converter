 ...
 *
 * <p>Note that the filter method is not thread safe
 *
...
public class ResampleOpSingleThread extends AdvancedResizeOp {
  ...
  private class SubSamplingData {
    ...
    private final int
        numContributors; // the primary index length for the 2Dim arrays : arrPixel and arrWeight
    private final float width;

    private SubSamplingData(
        int[] arrN, int[] arrPixel, float[] arrWeight, int numContributors, float width) {
      this.arrN = arrN;
  ...
  public BufferedImage doFilter(
    ...
    // Idea: Since only a small part of the buffer is used, scaling the image, we reuse the rows to
    // save memory
  ...
  /**
   * Pseudocode: for each for in destination image check that all dependent rows in source image is
   * scaled horizontally and stored in work pixels if not then scale missing rows horizontal and
   * store them in work pixels Scale the destination row vertically and store the result in out
   * pixels
   *
   * <p>It may seem counter intuitive that the vertical scale is done for each row. The simple
   * scaling algorithm would first scale the image horizontal (a row at a time) into the temp image,
   * and then scale the temp image vertically (a column at a time) into the final image. The
   * disadvantage of the simple approach is that you need a large temporary image.
   *
   * <p>I have avoided this by doing the vertically scale a row at a time. Scaling a single row
   * vertically, needs the horizontally scaled rows that the scaling depends on. These dependencies
   * will be lazy initialized. Since we know the height of the 'window' we work on (the maximum
   * number of source rows needed to calculate a dest row), the work pixels only needs to have the
   * same height.
   *
   * <p>Instead of creating a temporary image with a height different from the source image's
   * height, I created a double array where inner array is repeated (so if the window height is 3
   * the first and the forth row is the same instance). This keeps algorithm a bit simpler (the
   * alternative would be to maintain a mapping between)
   *
  ...
  private void scale(BufferedImage srcImg, byte[][] workPixels, BufferedImage outImage) {
    final int[] tempPixels =
        new int
            [srcWidth]; // Used if we work on int based bitmaps, later used to keep channel values
    final byte[] srcPixels =
            ...
            for (int channel = nrChannels - 1; channel >= 0; channel--) {
              ...
              for (int i = dstWidth - 1; i >= 0; i--) {
                ...
                for (int jj = horizontalMax - 1; jj >= 0; jj--) {
                  ...
                  sample +=
                      tempPixels[horizontalSubsamplingData.arrPixel[horizontalIndex]]
                          * horizontalSubsamplingData.arrWeight[horizontalIndex];
                  horizontalIndex++;
                }
      ...
      for (int x = 0; x < dstWidth; x++) {
        ...
        for (int channel = nrChannels - 1; channel >= 0; channel--) {
          ...
          for (int j = max - 1; j >= 0; j--) {
            int valueLocation = verticalSubsamplingData.arrPixel[index];
            sample +=
                (workPixels[valueLocation][xLocation + channel] & 0xff)
                    * verticalSubsamplingData.arrWeight[index];
