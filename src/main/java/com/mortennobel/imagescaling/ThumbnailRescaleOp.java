...
/**
 * The idea of this class is to provide fast (and inaccurate) rescaling method suitable for creating
 * thumbnails.
 *
 * <p>Note that the algorithm assumes that the source image is significant larger than the
 * destination image
 */
public class ThumbnailRescaleOp extends AdvancedResizeOp {
  ...
  protected BufferedImage doFilter(
    ...
    if (dest != null && dstWidth == dest.getWidth() && dstHeight == dest.getHeight()) {
      ...
      out =
          new BufferedImage(
              dstWidth,
              dstHeight,
              numberOfChannels == 4 ? BufferedImage.TYPE_INT_ARGB : BufferedImage.TYPE_INT_RGB);
    }
